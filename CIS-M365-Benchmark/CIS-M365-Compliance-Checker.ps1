...
function Connect-M365Services {
    ...
    try {
        ...
        Write-Log "Connecting to Power BI..." -Level Info
        Connect-PowerBIServiceAccount -ErrorAction Stop | Out-Null
        Write-Log "Connected to Power BI" -Level Success
...
function Test-PowerBI {
    ...
    # Pre-fetch all Power BI tenant settings in a single API call to optimize performance
    $cachedPBISettings = $null
    try {
        Write-Log "Retrieving Power BI Tenant Settings via REST API..." -Level Info
        
        # Requires MicrosoftPowerBIMgmt module and an active connection via Connect-PowerBIServiceAccount
        # Call the Power BI admin REST API to get all tenant settings
        $response = Invoke-PowerBIRestMethod -Url "admin/tenantsettings" -Method GET -ErrorAction Stop
        $cachedPBISettings = ($response | ConvertFrom-Json).tenantSettings

        if ($null -eq $cachedPBISettings) {
            Write-Log "Warning: Tenant settings object is null. Ensure you have the Power BI Administrator role." -Level Warning
        }
        else {
            Write-Log "Successfully retrieved $($cachedPBISettings.Count) Power BI tenant settings." -Level Success
        }
    }
    catch {
        Write-Log "Warning: Could not retrieve Power BI Tenant Settings. Verify your Power BI Admin permissions. Related checks will report errors." -Level Warning
    }

    # Helper function to find a specific setting by its exact settingName property
    function Get-PBISettingValue {
        param([string]$settingName)
        if ($null -eq $cachedPBISettings) { return $null }
        return $cachedPBISettings | Where-Object { $_.settingName -eq $settingName }
    }

    # Helper function to check if a setting is restricted
    # Returns $true if the setting is disabled OR if it's enabled but scoped/restricted to specific security groups
    function Test-PBISettingRestricted {
        param($setting)
        if ($null -eq $setting) { return $false }
        
        # Check if it is disabled entirely
        if ($setting.enabled -eq $false) { return $true }
        
        # If enabled, check if it's restricted to specific security groups instead of the entire organization
        if ($setting.enabled -eq $true -and $setting.securityGroupRules -and $setting.securityGroupRules.Count -gt 0) {
            return $true
        }
    ...
    try {
        ...
        # The exact API setting name for guest access
        $setting = Get-PBISettingValue "B2B_AllowGuestToAccessPowerBI" 

        if ($null -eq $setting) { throw "Setting 'B2B_AllowGuestToAccessPowerBI' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.1" -ControlTitle "Ensure guest user access is restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "Guest access is disabled or restricted to specific security groups"
        }
        else {
            Add-Result -ControlNumber "9.1.1" -ControlTitle "Ensure guest user access is restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Guest access is enabled for the entire organization" `
                -Remediation "Disable or restrict guest user access to Power BI in Tenant Settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.1" -ControlTitle "Ensure guest user access is restricted" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "B2B_AllowGuestToInviteExternalUsers" 

        if ($null -eq $setting) { throw "Setting 'B2B_AllowGuestToInviteExternalUsers' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.2" -ControlTitle "Ensure external user invitations are restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "External user invitations are disabled or restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.2" -ControlTitle "Ensure external user invitations are restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Users can freely invite external users" `
                -Remediation "Restrict the ability to invite external users in Power BI Admin Portal > Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.2" -ControlTitle "Ensure external user invitations are restricted" `
    ...
    try {
        ...
        # Checking if external users can browse the tenant
        $setting = Get-PBISettingValue "AllowExternalUsersToBrowseTenant" 

        if ($null -eq $setting) { throw "Setting 'AllowExternalUsersToBrowseTenant' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.3" -ControlTitle "Ensure guest access to content is restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "Guest browsing of content is restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.3" -ControlTitle "Ensure guest access to content is restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Guest browsing is enabled for the entire organization" `
                -Remediation "Restrict guest browsing of Fabric content in Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.3" -ControlTitle "Ensure guest access to content is restricted" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "PublishToWeb" 

        if ($null -eq $setting) { throw "Setting 'PublishToWeb' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.4" -ControlTitle "Ensure 'Publish to web' is restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "Publish to web is disabled or restricted to specific groups"
        }
        else {
            Add-Result -ControlNumber "9.1.4" -ControlTitle "Ensure 'Publish to web' is restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Publish to web is enabled for everyone" `
                -Remediation "Disable or restrict 'Publish to web' feature in Tenant settings > Export and sharing"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.4" -ControlTitle "Ensure 'Publish to web' is restricted" `
    ...
    try {
        Write-Log "Checking 9.1.5 - R and Python visuals restricted" -Level Info
        $rSetting = Get-PBISettingValue "RVisuals" 
        $pythonSetting = Get-PBISettingValue "PythonVisuals" 

        if ($null -eq $rSetting -or $null -eq $pythonSetting) { throw "R/Python Visual settings not found or API inaccessible." }

        # Both settings must be restricted
        if ((Test-PBISettingRestricted -setting $rSetting) -and (Test-PBISettingRestricted -setting $pythonSetting)) {
            Add-Result -ControlNumber "9.1.5" -ControlTitle "Ensure 'Interact with and share R and Python' visuals is 'Disabled'" `
                -ProfileLevel "L2" -Result "Pass" -Details "R and Python visuals are disabled or appropriately restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.5" -ControlTitle "Ensure 'Interact with and share R and Python' visuals is 'Disabled'" `
                -ProfileLevel "L2" -Result "Fail" -Details "R and Python visuals are currently enabled for the organization" `
                -Remediation "Disable R and Python script visuals in Power BI Admin Portal > Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.5" -ControlTitle "Ensure 'Interact with and share R and Python' visuals is 'Disabled'" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "EnableSensitivityLabels" 

        if ($null -eq $setting) { throw "Setting 'EnableSensitivityLabels' not found or API inaccessible." }

        # For this control, we want it to be ENABLED (True)
        if ($setting.enabled -eq $true) {
            Add-Result -ControlNumber "9.1.6" -ControlTitle "Ensure 'Allow users to apply sensitivity labels for content' is 'Enabled'" `
                -ProfileLevel "L1" -Result "Pass" -Details "Sensitivity labels feature is enabled"
        }
        else {
            Add-Result -ControlNumber "9.1.6" -ControlTitle "Ensure 'Allow users to apply sensitivity labels for content' is 'Enabled'" `
                -ProfileLevel "L1" -Result "Fail" -Details "Sensitivity labels feature is disabled" `
                -Remediation "Enable sensitivity labels in Power BI Admin Portal > Tenant settings > Information protection (requires MIP prerequisites)"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.6" -ControlTitle "Ensure 'Allow users to apply sensitivity labels for content' is 'Enabled'" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "ShareToAnyone" 

        if ($null -eq $setting) { throw "Setting 'ShareToAnyone' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.7" -ControlTitle "Ensure shareable links are restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "Shareable links ('People in your organization') are restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.7" -ControlTitle "Ensure shareable links are restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Shareable links are fully enabled" `
                -Remediation "Restrict 'People in organization' shareable links in Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.7" -ControlTitle "Ensure shareable links are restricted" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "AllowExternalDataSharing" 

        if ($null -eq $setting) { throw "Setting 'AllowExternalDataSharing' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.8" -ControlTitle "Ensure enabling of external data sharing is restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "External dataset sharing is restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.8" -ControlTitle "Ensure enabling of external data sharing is restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "External dataset sharing is permitted for the entire organization" `
                -Remediation "Restrict external dataset sharing in Power BI Admin Portal > Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.8" -ControlTitle "Ensure enabling of external data sharing is restricted" `
    ...
    try {
        Write-Log "Checking 9.1.9 - Block ResourceKey Authentication enabled" -Level Info
        $setting = Get-PBISettingValue "BlockResourceKeyAuthentication" 

        if ($null -eq $setting) { throw "Setting 'BlockResourceKeyAuthentication' not found or API inaccessible." }

        # For this control, the blocking feature must be ENABLED (True)
        if ($setting.enabled -eq $true) {
            ...
            Add-Result -ControlNumber "9.1.9" -ControlTitle "Ensure 'Block ResourceKey Authentication' is 'Enabled'" `
                ...
                -Remediation "Enable blocking of resource key authentication in Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.9" -ControlTitle "Ensure 'Block ResourceKey Authentication' is 'Enabled'" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "AllowServicePrincipalsToUsePowerBIAPIs" 

        if ($null -eq $setting) { throw "Setting 'AllowServicePrincipalsToUsePowerBIAPIs' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.10" -ControlTitle "Ensure access to APIs by Service Principals is restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "Service principal API access is restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.10" -ControlTitle "Ensure access to APIs by Service Principals is restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Service principals can freely access Power BI APIs" `
                -Remediation "Restrict service principal API access in Power BI Admin Portal > Tenant settings > Developer"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.10" -ControlTitle "Ensure access to APIs by Service Principals is restricted" `
    ...
    try {
        Write-Log "Checking 9.1.11 - Service Principal profile creation restricted" -Level Info
        $setting = Get-PBISettingValue "AllowServicePrincipalsToCreateProfiles" 

        if ($null -eq $setting) { throw "Setting 'AllowServicePrincipalsToCreateProfiles' not found or API inaccessible." }

        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.11" -ControlTitle "Ensure Service Principals cannot create and use profiles" `
                -ProfileLevel "L1" -Result "Pass" -Details "Service principal profile creation is restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.11" -ControlTitle "Ensure Service Principals cannot create and use profiles" `
                -ProfileLevel "L1" -Result "Fail" -Details "Service principals can create profiles freely" `
                -Remediation "Restrict service principal profile creation in Power BI Admin Portal > Tenant settings"
        }
    }
    catch {
        Add-Result -ControlNumber "9.1.11" -ControlTitle "Ensure Service Principals cannot create and use profiles" `
    ...
    try {
        ...
        $setting = Get-PBISettingValue "AllowServicePrincipalsToCreateWorkspaces" 

        if ($null -eq $setting) { throw "Setting 'AllowServicePrincipalsToCreateWorkspaces' not found or API inaccessible." }
        ...
        if (Test-PBISettingRestricted -setting $setting) {
            Add-Result -ControlNumber "9.1.12" -ControlTitle "Ensure service principals ability to create workspaces, connections and deployment pipelines is restricted" `
                -ProfileLevel "L1" -Result "Pass" -Details "Service principal workspace creation is disabled or restricted"
        }
        else {
            Add-Result -ControlNumber "9.1.12" -ControlTitle "Ensure service principals ability to create workspaces, connections and deployment pipelines is restricted" `
                -ProfileLevel "L1" -Result "Fail" -Details "Service principals have unrestricted access to create workspaces/pipelines" `
...
function Start-ComplianceCheck {
    ...
    try {
        ...
        Disconnect-PowerBIServiceAccount -ErrorAction SilentlyContinue | Out-Null # <--- AGGIUNGI QUESTA
    }
    catch {
